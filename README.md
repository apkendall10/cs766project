...
In our second phase, we address the issue of spatially varying blur. In natural settings, blurs are rarely applied uniformly to an image. Instead the blur is typically local. For example, blur caused by varying depth of field will only be present in the parts of the scene that differ in depth from the plane of focus. 

We used the true vs. blur dataset generated by Nah[7]. This dataset consists of pairs of blurry and sharp images. The blurry images have spatially varying blur. It is important to note that unlinke in phase I, we do not have a known blur kernel, and it is not limited to gaussian blur. Also, to improve training performance, we shrunk each image by a factor of 5.
